...
def register_pending_delete(user_id: str) -> None:
    ...
    with _pending_deletes_lock:
        _pending_deletes[user_id] = time.time()
        # LRU eviction
...
def is_pending_delete(user_id: str) -> bool:
    ...
    with _pending_deletes_lock:
        # cleanup רשומות פגות במקביל
        stale = [uid for uid, ts in _pending_deletes.items() if ts < cutoff]
        for uid in stale:
            del _pending_deletes[uid]
        return user_id in _pending_deletes


def clear_pending_delete(user_id: str) -> None:
    """מנקה state אחרי ביצוע או ביטול מפורש."""
    with _pending_deletes_lock:
        _pending_deletes.pop(user_id, None)
