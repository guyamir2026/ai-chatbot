...
> **סטטוס:** מסמך תכנון (spec) שאושר והמימוש שלו החל. נכתב על סמך חקירת עומק של הקוד (יולי 2026).
...
## החלטות שהתקבלו (יולי 2026)

| החלטה | בחירה |
|---|---|
| מסלול DB | **מסלול ב' — SQLite פר-tenant** + control plane (פרק 2.5) |
| טופולוגיית Twilio | **BYO פר לקוח** (כמו היום; שאלה פתוחה #2) |
| לקוחות פעילים | **אין כרגע** ⇒ שלב 4 (הגירת לקוחות קיימים) מתייתר; אין דאטה חיה להגן עליה בזמן הבנייה |

### סטטוס מימוש

- ✅ **שלב 1 — יסודות בקוד הקיים (אפס שינוי התנהגות):**
  - איחוד ה-namespace: `ai_chatbot/` הפך ל-alias namespace (meta-path finder ב-`ai_chatbot/__init__.py`); קבצי ה-wrapper נמחקו; `wsgi.py` עבר ל-`admin/wsgi.py`. אין יותר שני עותקים של module state (סעיף 1.6).
  - `tenancy.py`: ‏TenantContext (contextvar), ‏`tenant_context()`, ‏ולידציית slug, ‏`tenant_db_path()`/`tenant_faiss_dir()` עם מיפוי `default` → הקבצים הקיימים, ודגל `TENANCY_STRICT` לשלב 2.
  - `get_connection()` פותח את קובץ ה-DB לפי ה-tenant הנוכחי. נקודות כניסה נטועות: Flask ‏`before_request`/`teardown_request`, ‏broadcast scheduler, ‏memory scheduler. (במצב webhook ה-context מתפשט מה-Flask thread ללולאת הבוט דרך `run_coroutine_threadsafe`, שמעתיק contextvars.)
  - זהות עסקית דרך `config.get_business_config()` בזמן-ריצה במקום ייבוא-קפוא של `BUSINESS_NAME` וחבריו (llm, handlers, ics, notifications, whatsapp_webhook, developer_report, admin — כולל context processor במקום 35 kwargs); ‏`build_system_prompt` מקבל `business_name` כפרמטר.
  - תיקוני שורש: ‏ProxyFix על ה-Flask app; ‏webhook של WhatsApp קורא ושומר את `To` כ-`provider_asset_id` (העוגן לזיהוי tenant — סעיף 6.2).
  - נדחה משלב 1 (בהיעדר לקוחות פעילים): גיבוי לילי ומעבר gunicorn — נכנסים בשלב 2 כחלק מה-entrypoint של הפלטפורמה.
- ⬜ **שלב 0 — פתרון ביניים Google (פרק 9):** פעולה ידנית ב-Google Cloud Console (publish ל-Production) — רלוונטי ברגע שיש לקוח מחובר-יומן.
- 🔶 **שלב 2 — הפלטפורמה (בעבודה):**
  - ✅ **Control plane** (`control_plane.py` + `platform.db`): טבלאות `tenants` / `tenant_routes` / `tenant_secrets` (Fernet **fail-closed** דרך `encrypt_field_strict`), מחזור חיים מלא (create/suspend/activate, יצירת tenant = תיקייה + `init_db` + seed), ו-CLI תפעולי (`platform_cli.py`).
  - ✅ **אכיפת סטטוס**: tenant מושעה/בהגירה ⇒ `TenantSuspendedError` בכל גישת DB; במצב `TENANCY_STRICT` גם tenant לא-רשום נחסם (הגנת typo). cache סטטוסים 30ש' עם אינבלידציה.
  - ✅ **מיפתוח state בזיכרון לפי tenant (סעיף 5.3)**: ‏FAISS registry עם LRU (במקום singleton), ‏query cache + דגלי stale + מנעולי rebuild פר-tenant, ‏vacation cache, ‏rate limiter, מכונת המצבים של WhatsApp, ‏follow-up store, ‏pending-deletes, ‏summarize locks, ‏deletion guard, ‏template cache, ‏Twilio client registry (סודות פר-tenant מה-control plane; **בלי fallback ל-env** עבור tenant שאינו default — מניעת שליחה בזהות עסק אחר).
  - ✅ **Schedulers פלטפורמתיים**: ‏broadcast + memory עוברים על כל ה-tenants הפעילים (fallback ל-default כשאין רישום — אפס שינוי התנהגות), כשל אצל אחד לא עוצר אחרים; ‏jobs של PTB עטופים ב-tenant context מפורש (עד המעבר לבוטים פר-tenant).
  - ✅ **ראוטינג נכנס (פרק 6, בלי Telegram):** ‏WhatsApp — נתיב `POST /webhook/whatsapp/t/<key>` פר-tenant (+ ‏`/status`), ‏resolve-לפני-אימות וחתימת Twilio מאומתת עם ה-auth token של ה-tenant עצמו, ‏cross-check של `To` מול המספר הרשום; ‏Meta — ‏resolve של `entry.id` מול ה-control plane (`meta_page_id`/`meta_ig_account`) עם fallback ל-legacy, כל entry מעובד תחת ה-context שלו; ‏Widget — ‏`embed.js?k=<key>` מגיש את ההגדרות מה-DB של ה-tenant ואופה את המפתח ל-payload, ‏`/widget/api/chat` עושה resolve לפי המפתח, ‏rate limit במפתח ‏(tenant, ip); עמודים ציבוריים — ‏`/t/<slug>/p/<id>` ו-`/t/<slug>/ics/<id>` מוגשים מה-DB הנכון (mושעה/לא-קיים ⇒ 404 אחיד), בוני URL מרכזיים ב-`public_urls.py` (כולל status callback פר-tenant לשידורים), ‏`platform_cli show-urls` מדפיס את הכתובות להדבקה ב-Twilio Console. ה-blueprint של WhatsApp נרשם תמיד (ההחלטה פר-בקשה, לא לפי env בזמן boot).
  - ✅ **‏Telegram multi-bot runtime (spec 6.1):** ‏`bot_registry.py` — ‏Applications פר-tenant על ה-bot loop המשותף, נבנות **עצלה** בהודעה הראשונה (tenant שנרשם בזמן ריצה עובד בלי restart), בלי JobQueue (העבודות ב-schedulers הפלטפורמתיים); ‏`POST /telegram/webhook/t/<key>` עם אימות secret פר-tenant ‏(fail-closed — בלי secret רשום אין שירות); ‏`bot_state` הפך ל-registry רב-tenant בלי fallback צולב (tenant בלי בוט ⇒ None, לא הבוט של אחר); שליחה יוצאת ב-live_chat (טלגרם HTTP API + שער WhatsApp) נפתרת לפי ה-tenant; ‏CLI ‏`connect-telegram` — מפתח+secret אידמפוטנטיים ו-setWebhook מול טלגרם (עם ‏initialize/shutdown תקינים ל-Bot עצמאי); כיבוי מסודר של בוטי tenants ב-atexit. הבוט ה-legacy (env) ממשיך ללא שינוי; ‏polling נשאר לפיתוח בלבד — הפלטפורמה רצה במצב webhook.
  - ✅ **אדמין רב-משתמשים:** טבלת `admin_users` ב-control plane (‏email מנורמל, ‏hash של werkzeug שלעולם לא מוחזר החוצה, ‏CHECK ש-owner קשור ל-tenant ו-platform_admin לא, אימות בלי timing oracle, בלי self-registration); ה-login מקבל גם email — משתמש פלטפורמה מקבל session עם `tenant_id`+`role` (עם ניקוי session נגד fixation), וה-`before_request` קושר את הפאנל כולו ל-tenant מה-session — **כל הפאנל הקיים הפך פר-tenant בלי לגעת באף route עסקי**; ‏owner של tenant שהושעה מנותק אוטומטית בבקשה הבאה; מסך `/platform` ל-platform admins (רשימת tenants + סטטוסים + חיבורים, "פעל-כ" שמעביר את הפאנל לעסק נבחר עם באנר קבוע, השעיה/הפעלה) — לאחרים 404; ‏CLI: ‏`create-admin` (סיסמה מ-stdin/prompt) / ‏`list-admins` / ‏`disable-admin`. ה-login ה-legacy מה-env ממשיך לעבוד (ה-tenant של ברירת המחדל).
  - ✅ **OAuth מרכזי + הקשחת ריצה + גיבוי לילי:**
    - **Google OAuth מרכזי:** הזרימה כבר tenant-safe מבנית — שני ה-routes ‏`@login_required`, ה-tenant נקבע מה-session (owner / "פעל-כ"), והטוקנים נשמרים ל-DB של אותו tenant (spec §3.2 — ‏binding לפי session מאומת, לא לפי `state`). ‏client_id/secret פלטפורמתיים ו-redirect יחיד — אין יותר אפליקציה/דומיין פר-לקוח. נוסף **keep-alive שבועי חוצה-tenants** (`refresh_all_tenant_calendars`) שמונע תפוגת "לא-בשימוש 6 חודשים" **ומגלה ניתוקים מוקדם** (מפעיל את התראת בעל העסק הקיימת). תוקנה דליפת email לאודיט (domain בלבד, דפוס #7) והוסר ייבוא קפוא של `BUSINESS_NAME`; ייבוא ה-oauthlib הפך lazy (המודול נטען בלי כל שרשרת ה-google client).
    - **הקשחת ריצה:** ‏`flask_app.run(threaded=True)` — ‏webhook איטי (RAG+LLM inline) לא מעכב אחרים. ‏gunidcorn multi-worker **לא** נכנס במכוון: הפריסה היא תהליך-יחיד (bot loop, schedulers, registries בזיכרון), וריבוי workers ידרוש externalization מלא של ה-state (Redis + scheduler חיצוני) — עבודה עתידית, לא בשלב הזה.
    - **גיבוי לילי:** ‏`backup_service` — העתקים **עקביים** דרך ה-online backup API של SQLite (בטוח בזמן WAL) לכל tenant + `platform.db` + אינדקסי FAISS; ‏prune לפי שם-תיקייה (חסין clock-skew); ‏seam מפורש (`set_upload_hook`) להעלאה ל-object storage. ‏`platform_maintenance` scheduler מריץ גיבוי יומי + keep-alive שבועי לפי last-run מתמיד (אידמפוטנטי בין restarts); כל איטרציה עטופה — כשל אצל tenant אחד לא עוצר את השאר.
  - **שלב 2 הושלם.** נותר לפרויקט: פעולת Google Console החד-פעמית (דומיין + Search Console + publish + הגשת verification, spec §3.4/§9), ופיילוט (שלב 3) עם לקוח אמיתי ראשון.
- 🔶 **שיפורי onboarding ומוצר (אחרי שלב 2):**
  - ✅ **הקלטת לקוח בלי CLI:** אשף "לקוח חדש" ב-`/platform` (יוצר tenant + owner), ותיקון-שורש של מסך "הגדרות תשתית" — סודות הערוצים (טלגרם/Twilio) נשמרים מוצפנים פר-tenant ב-control plane במקום ב-`.env` הגלובלי (הבאג שהיה דורס tenants), עם connect-telegram מהפאנל. ה-tenant של ברירת המחדל (legacy) נשאר על env.
  - ✅ **מודל LLM כשדרוג:** `llm_model` פר-חבילה (`plans_config`) + `feature_flags.get_llm_model()` (סדר: env override `PLAN_LLM_MODEL_<PLAN>` → מודל החבילה → `OPENAI_MODEL`), מחווט ל-`llm.py`. ברירת המחדל ריקה ⇒ אפס שינוי התנהגות עד שמגדירים מודל לחבילה; שדרוג = חבילה עם מודל חזק יותר. הערה: תמחור מדויק דורש מדידת צריכת טוקנים פר-tenant (שאלה פתוחה #3) — טרם מומש.
  - ✅ **זהות עסקית + ערוץ פר-tenant (סגירת 7 פערי ה-onboarding):**
    - **זהות עסקית פר-tenant:** ‏`get_business_config()` השלים את "שלב 2" שלו. **שם העסק** — מקור-אמת יחיד: `display_name` ב-control plane (נגזר בזמן-ריצה, בלי עותק שקופא, בלי שדה כפול לעריכה). **כרטיס ביקור** (טלפון/כתובת/אתר) — עמודות ב-`bot_settings`, נערכות במסך ייעודי חדש **"כרטיס ביקור"** (`/business-card`); שם העסק מוצג שם לקריאה בלבד. ריק ⇒ fallback ל-env (legacy ללא שינוי). ‏widget (`_build_widget_config`) עבר גם הוא ל-accessor. ⇒ הפרומפט/vCard/ICS/manifest של כל tenant מציגים את העסק הנכון, לא את env הפלטפורמה.
    - **ערוץ כמאפיין tenant עם נעילה אוטומטית:** ‏`subscription.channel` (‏''/telegram/whatsapp; ‏`feature_flags.get_channel/set_channel`), **הוסר `channel` מ-PLANS** (וגם התראת ה-mismatch מ-`developer_alerts`/`main` — איבדה נקודת ייחוס). ‏tenant טרי — שני מקטעי הערוצים פתוחים; חיבור מוצלח ראשון (טוקן טלגרם / שלישיית Twilio) קובע את הערוץ ונועל את השני; שחרור — כפתור "שחרר ערוץ" ב-`/platform` ‏(platform admin בלבד); עם חיבור הערוץ החדש נתוני הערוץ הקודם נמחקים (`control_plane.delete_tenant_channel_data` — סודות+ראוטים; ביטול webhook טלגרם best-effort לפני מחיקת הטוקן). ה-default פטור (env). ‏preview הפרומפט ומסך "החבילה שלי" מציגים את ערוץ ה-tenant.
    - **"פעל-כ" מציג ומעדכן את ה-owner:** ‏`control_plane.get_tenant_owner()`; מסך הגישה מציג את אימייל ה-owner של ה-tenant (לא של המשתמש המחובר), ושינוי סיסמה מאפס את סיסמת ה-owner — גם במצב פעל-כ.
    - **QR ו-widget פר-tenant:** ‏`get_tenant_channel_identity()` ‏(default⇒env, אחרת סודות); שם המשתמש של הבוט נלכד ב-getMe בעת connect (פאנל + CLI) ונשמר כ-secret ‏`telegram_bot_username`; מספר ה-WhatsApp מוזן ישירות ונשמר כ-`twilio_whatsapp_number` (אין getMe ל-WhatsApp — אין מה לגלות); עמוד ה-QR בונה t.me/wa.me מהזהות של ה-tenant; ‏`/widget-embed` מנפיק `?k=<widget_key>` אוטומטית בביקור הראשון ו-`/widget/demo` מקבל `?k=`.
    - **תצוגת ערוץ פר-tenant בלי env:** ה-preview של הפרומפט ומסכי החבילה גוזרים את הערוץ מ-`feature_flags.get_channel()`, ובנפילה — מ-`_detect_tenant_channel` שקורא את **סודות ה-control plane** של ה-tenant (ל-default בלבד: env). לא נשענים יותר על `detect_active_channel` (env) ל-tenant בפלטפורמה, שה-credentials שלו לא ב-env.
    - **Meta — הסתרה מבעל העסק:** שדות ה-config הפלטפורמתיים (Webhook URL / Verify Token / OAuth Redirect) מוסתרים מ-tenant בפלטפורמה — הם ברמת האפליקציה המשותפת ומנוהלים ע"י ספק השירות; נשאר רק חיבור העמוד/החשבון דרך OAuth. ל-tenant של ברירת המחדל (env admin) הם עדיין מוצגים.
    - ‏rollout ל-tenant קיים: שמירת הטוקן מחדש לוכדת את שם הבוט; טלפון/כתובת/אתר ממולאים פעם אחת במסך "כרטיס ביקור".
- ⬜ **שלב 3 — פיילוט.** שלב 4 ברובו מתייתר (אין לקוחות קיימים); שלב 5 — סגירה.
