...
def _build_channel_rules(channel: str, booking_enabled: bool = True) -> str:
    """בניית כללים 5-7 בהתאם לערוץ — טלגרם (כפתורים) או WhatsApp (טקסט).

    booking_enabled=False: העסק אינו מתאם תורים אונליין — כלל 5 מפנה
    בקשות תור/פגישה לנציג במקום לכפתור/flow של קביעת תור.
    """
    # ── כלל ההעברה לנציג ──
    ...
    if channel == "whatsapp":
        if booking_enabled:
            booking_line = "5. אם הלקוח רוצה לקבוע תור או פגישה, בקש ממנו לכתוב את השירות שהוא מעוניין בו ותסייע לו בקביעה.\n"
        else:
            booking_line = (
                "5. העסק אינו מתאם תורים/פגישות אונליין. אם הלקוח מבקש לקבוע תור או פגישה, "
                "התייחס לכך כבקשה לדבר עם בעל העסק והפעל את כלל 7 (ההעברה לנציג).\n"
            )
        return (
            f"{booking_line}"
    ...
    if channel == "widget":
        ...
        return (
            "5. אם המבקר רוצה לקבוע תור או פגישה, לדבר עם נציג, או שיחזרו אליו "
    ...
    if booking_enabled:
        booking_line = "5. אם הלקוח רוצה לקבוע תור או פגישה, הנחה אותו להשתמש בכפתור בקשת התור.\n"
    else:
        booking_line = (
            "5. העסק אינו מתאם תורים/פגישות אונליין. אם הלקוח מבקש לקבוע תור או פגישה, "
            "התייחס לכך כבקשה לדבר עם בעל העסק והפעל את כלל 7 (ההעברה לנציג).\n"
...
**open_issue פתוחים:** אם יש fact מסוג open_issue (החזר, תלונה, וכו'), זה אומר שיש משהו שעוד לא נסגר. תוכל להזכיר את זה אם רלוונטי, או לבדוק אם הוא נפתר."""
    ...
    # כלל 8 — הצעת פעולות. כשקביעת תורים כבויה לעסק, לא מציעים "לבקש תור".
    if booking_enabled:
        booking_suggestion_rule = '8. הצע פעולות רלוונטיות בהתאם (לדוגמה, "האם תרצו לבקש תור?").'
    else:
        booking_suggestion_rule = "8. הצע פעולות רלוונטיות בהתאם (למשל, להעביר את הפנייה לבעל העסק כשצריך)."
...
{_build_channel_rules(channel, booking_enabled)}
{booking_suggestion_rule}
